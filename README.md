# Учебный курс (Работа с программным продуктом Isaac GR00T с использованием Isaac Sim)

Репозиторий содержит учебно-методические материалы и программные модули учебного курса по работе с обобщённой моделью управления роботами **Isaac GR00T** в средах **Isaac Sim** и **Mujoco**. Курс разработан в рамках дисциплины «Прикладные интеллектуальные системы» (Кафедра ИИТ, БрГТУ, 2026).
...
```bash
git clone [https://github.com/AndyWorld26/Isaac-Gr00t](https://github.com/AndyWorld26/Isaac-Gr00t)
cd Isaac-Gr00t
...
**Цель занятия:** Формирование устойчивых навыков работы с графической средой Isaac Sim, импорт робототехнических USD-активов и интерактивное управление сочленениями через GUI.
...
1. **Клонирование репозитория симулятора и запуск:**
```
git clone -b main [https://github.com/isaac-sim/IsaacSim.git](https://github.com/isaac-sim/IsaacSim.git) isaacsim
cd isaacsim
git lfs install
git lfs pull
build.bat
...
1. **Настройка окружения:** Изоляция потоков данных и исключение накладных расходов GUI-интерфейса осуществляется за счет запуска через оптимизированный скриптовый мост локомоции.
...
```
3. Сконфигурировать переменные окружения `PYTHONPATH` и устранить возможные конфликты версий зависимых пакетов, установленных из requirements.txt
4. Запустить скрипт интеграции и убедиться, что робот-гуманоид Unitree G1 успешно инициализируется в устойчивой вертикальной стойке (T-pose).
...
Для итоговой защиты проекта подготовлен демонстрационный плакат, отражающий ключевые аспекты разработки:
* **Название проекта** 
* **Цель и актуальность** 
...
**[Открыть демонстрационный плакат проекта (PDF)](./video/plakat.pdf)**
